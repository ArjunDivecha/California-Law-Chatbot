...
// ---------------------------------------------------------------------------
// New patterns (W3, W8, W9, audit §8 items 4-6)
// ---------------------------------------------------------------------------

await test('Dollar amount: $-prefixed and verbal "X million dollars"', () => {
  mustDetect(DOLLAR_AMOUNT, [
    'client claims $237,500 in damages',
    'settled for $4.3M',
    'estate valued at $1,234.56',
    'awarded 2 million dollars',
  ]);
  mustNotDetect(DOLLAR_AMOUNT, [
    'Cal. Civil Code § 3294',
    'see section 12345',
  ]);
});

await test('California bar number: SBN and Bar No. formats', () => {
  mustDetect(CA_BAR_NUMBER, [
    'SBN 123456',
    'Bar No. 234567',
    'Cal. Bar # 345678',
    'State Bar Number: 456789',
  ]);
  mustNotDetect(CA_BAR_NUMBER, [
    'the bar is open',
    'raise the bar higher',
  ]);
});

await test('California court case: BC-style and year-prefix formats', () => {
  mustDetect(CA_COURT_CASE, [
    'filed as BC712345 in LA Superior',
    'matter 23STCV12345',
  ]);
  mustNotDetect(CA_COURT_CASE, [
    'Cal. Civ. Code § 3294',
    'hello world',
  ]);
});

await test('ISO date: YYYY-MM-DD is caught', () => {
  mustDetect(DATE_ISO, [
    'DOB: 1985-06-15',
    'filing date 2024-01-15',
    '2023-12-31',
  ]);
  mustNotDetect(DATE_ISO, ['no date here', 'section 2024']);
});

await test('Verbose date: spelled-out month formats', () => {
  mustDetect(DATE_VERBOSE, [
    'born on January 15, 2024',
    '15 January 2024',
    'Jan. 15, 2024',
  ]);
  mustNotDetect(DATE_VERBOSE, [
...
await test('analyze: privileged=true when high-risk span detected', () => {
  const r1 = analyzer.analyze('My client Maria Esperanza needs help.');
  assert.ok(r1.privileged, 'name → privileged');
  const r2 = analyzer.analyze('Contact: (415) 555-0123');
  assert.ok(r2.privileged, 'phone → privileged');
  const r3 = analyzer.analyze('dispute involves $4.3M');
  assert.ok(r3.privileged, 'dollar amount → privileged');
  const r4 = analyzer.analyze('What are trustee duties under California law?');
  assert.ok(!r4.privileged, 'general legal question → not privileged');
});

await test('analyze: confidence=1.0 for deterministic-only detections', () => {
  const r = analyzer.analyze('SSN: 123-45-6789');
  assert.ok(r.spans.some(s => s.category === 'ssn'), 'SSN detected');
  assert.strictEqual(r.confidence, 1.0, 'deterministic pattern → confidence 1.0');
});

await test('analyze: confidence<1 when heuristic name signals fire', () => {
  const r = analyzer.analyze('my client Maria Esperanza filed a claim');
  assert.ok(r.confidence < 1.0, 'heuristic name signal reduces confidence');
  assert.ok(r.confidence > 0.5, 'still in a plausible range');
});
