...
- **[Low]** Stale doc-comment: `RunTurnOptions.model` says *"defaults to claude-sonnet-5"* while `DEFAULT_MODEL` is now `claude-fable-5`. Minor, but in a system that treats prompts/config as legal artifacts, stale comments are a real liability.
