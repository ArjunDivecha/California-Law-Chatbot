# CLAUDE.md

Operator's manual for coding agents working in this repo. Global rules (light mode,
doc headers, `file://` result links) live in `~/CLAUDE.md` and are not repeated here.
Regenerated by Fable 5 on 2026-07-06 from the current V4 code — the previous CLAUDE.md
described the retired V1 (OpenRouter/Gemini/CEB) architecture and was fully stale.

## Purpose

**DancingElephant** (rebranded from AskPauli 2026-08-18; repo/Vercel project name is
still `california-law-chatbot`) is a Clerk-authenticated legal research +
document-drafting assistant for California attorneys, being commercialized beyond its
original firm (Femme & Femme Law). The current mainline is **V4**: a single `/v2` front
end backed by Vercel serverless functions that run a server-side agent loop calling the
**Anthropic Messages API directly**, gated by a server-authoritative compliance policy
engine, with **on-device PII tokenization** before any text leaves the browser. It is
deployed and in production at `dancingelephant.ai`.

**Brand/design**: violet #7C5CFC / teal / amber tokens live in `tailwind.config.cjs`
(`theme.extend` is no longer empty); Inter (UI) + Fraunces (display); Georgia only
inside rendered legal documents (`font-doc`); gradient motif classes `.de-thinkline` /
`.de-rule` / `.de-spinner` in `index.html`. Design source of truth:
`docs/design-handoff/` (9 artboards + spec README). No pink, no emoji chrome, no
"AskPauli"/"Pauli" strings in UI.

## Deep architecture: read OpenWiki first

`openwiki/quickstart.md` → `openwiki/architecture.md` / `workflows.md` / `domain-model.md`
(dated 2026-07-04) are accurate and current. Read them for the full request flow, trust
boundary, and per-workflow file maps. This file covers only what OpenWiki does not:
verified commands, data locations, gotchas, and current broken state.

## Architecture map (load-bearing files, absolute paths)

Base: `/Users/arjundivecha/Dropbox/AAA Backup/A Working/California-Law-Chatbot`

- `api/_lib/agentLoop.ts` — the turn engine; the only code that calls Anthropic Messages. Bounds iterations, dispatches tools, computes per-turn policy, writes audit/manifest. (1661 lines — biggest server module.)
- `api/_lib/agentProxy.ts` — entry every `/api/agent/*` route calls; runs the fail-closed server-side PII regex backstop before delegating to `agentLoop`.
- `api/_lib/compliance/policyEngine.ts` — `decidePolicy()`, the server-authoritative gate: matter mode, tool allow/block, consent hard-blocks, disclosures, review gates.
- `api/_lib/compliance/toolQueryGuard.ts` — last-mile outbound exfiltration guard (fail-closed: every branch defaults to `allowed:false`).
- `api/_lib/tools/index.ts` — tool registry / `dispatchTool()`; per-tool try/catch returns `is_error` tool_results instead of throwing.
- `api/_lib/httpGuard.ts` — Clerk auth (`requireUser`), CORS allowlist, session-ownership, rate limiting; shared by all agent routes.
- `api/_lib/sessionStore.ts` — Redis-shaped store wrapper (Turso/libSQL or Upstash): session messages/meta, tool-result cache, single-flight turn lock, rate-limit counter.
- `api/_lib/libsqlKv.ts` — Turso/libSQL implementation of `SessionRedis` + `AuditSink` + `EnvelopeSink`; hosted twin of the desktop `sqliteKv.ts`. Test: `tests/libsqlKv.test.mjs` (local `file:` DB).
- `api/_lib/approvedModels.ts` — counsel-approved Anthropic model allowlist; `assertApprovedModel` throws (fail-closed) on any non-listed id.
- `api/_shared/sanitization/patterns.ts` — deterministic PII regexes (SSN, phone, card, `CA_DRIVER_LICENSE`, …); shared by both sanitizer pipelines.
- `api/_shared/sanitization/index.ts` — `analyze()` span analyzer (the "analyze pipeline").
- `services/sanitization/detectionPipeline.ts` — `detectPii()` the browser runs before `fetch()` (the "wire pipeline"); imports the same `runPatterns`.
- `services/sanitization/wireGuard.ts` — final `assertNoRawPii()` regex assert before send.
- `agents/california-legal/agent.json` (+ skill markdown) — agent config: model, `max_iterations`, system prompt source.
- `App.tsx` / `index.tsx` — SPA boot + router; routes all traffic into four `/v2` surfaces (`components/v2/V2{Chat,Draft,Verify,DraftingMagic}Page.tsx`).

## Commands that work

Package manager is **yarn 4** (`packageManager: yarn@4.9.1`), Node **v24**. Canonical
scripts are in `package.json`; below, "verified" = I ran the underlying binary and it
exited 0 on 2026-07-06.

```bash
yarn build              # vite build → dist/  — VERIFIED (exit 0, ~5s; dist/ is gitignored)
yarn test:sanitization  # tsx tests/sanitization.test.mjs — VERIFIED (153 passed, 0 failed)
yarn test:traps         # tsx tests/traps/runTraps.mjs — VERIFIED (100/100 analyze traps)
# All 15 tests/*.test.{mjs,ts} pass when run directly via ./node_modules/.bin/tsx — VERIFIED

# Wire-pipeline trap runner (not wired to any npm script) — GREEN as of
# 2026-07-17 (120/120, exit 0; the T-PII-032 regression was fixed in 40c9881):
./node_modules/.bin/tsx tests/traps/runTrapsWire.mjs

yarn dev                # vite dev server, http://localhost:5173  (unverified this run)
yarn dev:api            # tsx dev-server.js — local API proxy      (unverified this run)
yarn dev:full           # dev:api + dev together                   (unverified this run)
```

**Do NOT run casually — these spend Anthropic/OpenAI credits and hit live APIs:**
`yarn agent:smoke`, `yarn agent:smoke-stream`, `yarn latency:baseline`
(`scripts/agent-loop-smoke.mjs`, `agent-loop-stream-smoke.mjs`, `latency-baseline.mjs`).

## Data locations (absolute paths)

- `…/California-Law-Chatbot/.env` — live secrets (ANTHROPIC, OPENAI, CLERK_SECRET_KEY, TURSO via `.env.preview.local`, …). **Gitignored**; only `.env.example` is tracked. Do not commit `.env`, `.env.local`, `.env.local.bak-*`, or `archive-env-2026-07-01/` (all gitignored).
- Sessions / metadata / audit / rate-limit — **Turso/libSQL** via `api/_lib/libsqlKv.ts` (`TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN`, required for the web build; same five-table schema as the desktop app's `api/_lib/desktop/sqliteKv.ts`). Key shapes in `api/_lib/sessionStore.ts` (schema doc `docs/upstash-kv-schema-v1.md` still describes them). **Upstash Redis is gone** (decommissioned 2026-09-15: env vars removed, `@upstash/redis` dropped, database emptied to 0 keys — the empty `cal-law-chat-redis` shell still needs a one-click delete in the Upstash console, as does the empty `california-law-ceb` vector index; the management API key cannot address resources by id); historical migration script `scripts/migrate-upstash-to-libsql.mjs`.
- Sanitization token maps + local UX state — **browser IndexedDB** (encrypted store `api/_shared/sanitization/store.ts`).
- Chat payload persistence — optional **Vercel Blob** via `api/chats.ts`.
- Trap fixtures — `…/tests/traps/manifest-v1.json`. Test/smoke reports land in `…/reports/` (timestamped JSON).
- Build output — `…/dist/` (gitignored, regenerated by `yarn build`).

## Conventions & gotchas (non-obvious, repo-specific)

1. **CEB RAG and all OpenRouter/Gemini routing are RETIRED** (2026-07-02/03 V1 purge). `ceb-search`, `gemini-chat`, `claude-chat`, `courtlistener-search`, `verify-citations`, etc. no longer exist as endpoints. `ceb_search` is permanently blocked (CEB Terms & Conditions prohibit ingesting their content — `policyEngine.ts` comment). Do not reintroduce.
2. **Anthropic-direct only.** Primary model `claude-fable-5` (`agentLoop.ts:71`, override `V2_PRIMARY_MODEL`); `claude-opus-5` is the *automatic failover* on a model-unavailable 404 (`:84`, `V2_FALLBACK_MODEL`), not a co-default; Quick mode and the citation verifier use `claude-sonnet-5`. Any new model id must be added to `approvedModels.ts` or the request throws.
3. **Two sanitizer implementations share one pattern set** (`api/_shared/sanitization/patterns.ts`): the analyze pipeline (`api/_shared/sanitization/index.ts`) and the wire pipeline (`services/sanitization/detectionPipeline.ts`). They can drift — that drift is exactly what produced the T-PII-032 wire regression (FABLE.md P0; fixed in 40c9881, re-verified green 2026-07-17). Keep them in sync and run **both** trap runners after any sanitization change.
4. **Gating is fail-CLOSED; audit/plumbing is deliberately fail-OPEN.** `policyEngine`, `toolQueryGuard`, and the PII backstop (→ 503) block on error. Rate limiting (`httpGuard.ts:206`), session-ownership on KV blip (`httpGuard.ts:186`), and audit/manifest writes (`api/_shared/auditLog.ts`) fail *open* by design and say so in comments — do not "harden" them into fail-closed without understanding the availability tradeoff.
5. **`public_research` matter mode lets raw PII reach Anthropic by design** (`agentLoop.ts:960-980`): the DPA-covered direct channel is treated as safe, so escalation only strips third-party tool egress, it does not hard-block the Anthropic call. Confidential/protected modes behave differently. This is intentional, not a leak — but know it before changing matter-mode logic.
6. **`api/chats.ts` reimplements Clerk auth locally** instead of using `httpGuard.requireUser` (duplicates `utils/auth.ts`). If you change auth, change it in both places.
7. **Test wiring gap / no CI test gate.** Only `test:sanitization` + `test:traps` are npm scripts; the other 14 test files and `runTrapsWire.mjs` have no entry point, and the only GitHub Action (`.github/workflows/openwiki-update.yml`) is a docs bot, not a test gate. Regressions can and did ship (see gotcha 3).
8. **Dual lockfiles**: `yarn.lock` is canonical; `package-lock.json` is stale — use yarn. Local dev has a `dev-user` auth bypass that is disabled whenever `process.env.VERCEL` is set or `NODE_ENV=production` (`httpGuard.ts`), so it cannot leak to prod.

## Current state

- **2026-09-15 — release-readiness audit (8 parallel reviews) says NOT release-ready.** Report + per-dimension findings are gitignored (they describe live weaknesses) at `reports/audit-2026-09-15/` — read `RELEASE-AUDIT-2026-09-15.html` first. Headline blockers: Clerk *development* instance in production with open public sign-up; no ToS/Privacy/DPA; no tenant boundary, sign-out, delete or export; PII allowlist substring bug and unenforced consent gate; citation verifier wrong in both directions; repo still public with firm memos and agent transcripts tracked; no CI. All 2026-09-02 P0s were still open at audit time.
- **2026-09-13 — repo consolidated to a single `main`.** All other branches (the July `codex/citelaw-workflow-validation` fork and its backup/publish twins, the merged `feature/*`/`fix/*` branches, `worktree-tauri-spike`) and all `.claude/worktrees/*` were deleted after salvaging their research artifacts into `main` (commit 0e4210a). Full pre-cleanup history: `/Users/arjundivecha/Dropbox/AAA Backup/Temp/California-Law-Chatbot-all-refs-2026-09-13.bundle`; the uncommitted July UI restyle is saved as `…/Temp/California-Law-Chatbot-uncommitted-ui-restyle-2026-09-13.patch`. Keep it that way: no long-lived branches or worktrees.
- **2026-09-13 — web session store moved from Upstash Redis to Turso/libSQL** (`api/_lib/libsqlKv.ts`, Marketplace resource `cal-law-chat-turso`, env `TURSO_*` in all three environments). 267 keys migrated and verified (`reports/upstash-cleanup-2026-09-13/migration-*.json`); production verified reading and writing Turso. **2026-09-15: Upstash Redis fully decommissioned** after a two-day soak with zero fallback writes — env vars removed from Vercel, `@upstash/redis` removed, fallback code paths deleted, `cal-law-chat-redis` emptied with FLUSHALL (final export in `reports/upstash-cleanup-2026-09-13/final-upstash-dump-*.json`). Compliance note: `providerRegistry.ts` still carries the retired `upstash_redis` entry and `storagePolicy.ts` the `cloud_upstash` target so counsel-reviewed policy semantics are unchanged — **counsel TODO: review a Turso entry (DPA, subprocessors, region)**.
- **Active, in production**: `https://dancingelephant.ai` (primary, DNS on Cloudflare — nameservers titan/zelda; zone-migration script `scripts/migrate-dns-cloudflare.mjs`), plus `california-law-chatbot.vercel.app` and `chat.femmeandfemmelaw.com` (Vercel auto-deploy from `main`). Legacy `askpauli.com` still serves the old AskPauli landing page (rebrand + redirect pending).
- Mainline is V4: one `/v2` front end, V1 fully purged. **DancingElephant rebrand shipped 2026-08-18** — all five surfaces reskinned, logic untouched; attestation checkbox now required; trust copy standardized on "Confidential client information never leaves your computer".
- **Sanitization suite fully green** (re-verified 2026-08-18 post-rebrand): `runTrapsWire.mjs` 120/120 zero-leak, `runTraps.mjs` 120/120, `sanitization.test.mjs` 153/153, `docx-surgery.test.mjs` 50/50. Note: `sanitization.test.mjs` asserts the attestation modal's rebrand headings — keep copy and test in sync.
- **Desktop app shipped**: notarized, Gatekeeper-accepted `DancingElephant.app` (installed in /Applications; installer zip in `installer-pkg/dist/`, **contains live API keys — private channels only**). Bundle id stays `com.askpauli.desktop` and the data dir stays `~/Library/Application Support/AskPauli/` deliberately — changing either orphans existing installs' local SQLite data. `yarn desktop` (dev) / `yarn desktop:app` (build+notarize).
- Rebrand gotchas: Vercel previews are SSO-protected and Vercel bot-protection challenges repeated headless-browser hits from this machine (verify prod-identical builds via `vite preview` instead); Clerk application name (sign-in card title) is dashboard-configured and still says "California Law Chatbot".
