...
- [Draft versioning and redline](draft-versioning.md) - The V2 Draft page version history subsystem (phases 1 and 2): an immutable per-session version chain in encrypted IndexedDB, word-level redline compare via diff-match-patch, device-local encrypted draft session persistence, and proposal-parsing recovery for truncated model replies.
