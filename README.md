...
>
> **V1 is fully purged (2026-07-02): ONE front end, ONE link.** `https://california-law-chatbot.vercel.app` — the root, the legacy `-v2` URL, and every old path all land on the V2 app (`/v2`). The V1 UI (ChatPage/Sidebar/useChat/gemini pipeline), all legacy OpenRouter + V1-only API routes, 24 root test scripts, and stale tech notes were deleted (recoverable from git history/archive tags). The platform-level wildcard CORS header in vercel.json was removed (per-route strict CORS governs), `/api/chats` regained its pre-save PII backstop, `/api/export-document` now requires Clerk auth, and the invented-token warning was ported to the V2 chat surface. Test battery fully green: sanitization 153/153, 12 compliance suites, traps 120/120 zero leaks.
>
